...
<!-- (DIPERBARUI) Memperbaiki semua bug dengan mengembalikan logika yang hilang. -->
<script setup>
...
// PERBAIKAN: Menambahkan kembali 'defineEmits'
const emit = defineEmits(['open-settings']);

const isMobileMenuOpen = ref(false);
const scrolled = ref(false);

// PERBAIKAN: Mengembalikan logika deteksi scroll
const handleScroll = () => {
  scrolled.value = window.scrollY > 10;
};

// PERBAIKAN: Mengembalikan semua logika status dan Spotify
const song = ref(null);
...
<template>
  <header class="header-container" :class="{ scrolled: scrolled }">
    <div class="header-content">
      ...
      <nav class="desktop-nav">
        <router-link to="/"><div class="link-text"><span class="en">Home</span><span class="jp">ホーム</span></div></router-link>
        <router-link to="/projects"><div class="link-text"><span class="en">Projects</span><span class="jp">プロジェクト</span></div></router-link>
        <router-link to="/mods"><div class="link-text"><span class="en">Mods</span><span class="jp">改造</span></div></router-link>
        <router-link to="/about"><div class="link-text"><span class="en">About</span><span class="jp">私について</span></div></router-link>
        <!-- PERBAIKAN: Menambahkan event handler @open -->
  ...
  <!-- PERBAIKAN: Menambahkan event handler @open-settings -->
  <MobileNav 
...
.profile-section { display: flex; align-items: center; gap: 0.75rem; overflow: hidden; }
.status-container { display: flex; flex-direction: column; overflow: hidden; }
...
.desktop-nav a:hover .jp, .desktop-nav a.router-link-exact-active .jp { color: var(--accent-color); opacity: 1; }
.link-text { display: flex; flex-direction: column; line-height: 1.3; }
.jp { font-size: 0.7rem; font-weight: 400; color: var(--text-color-secondary); opacity: 0.7; transition: color 0.2s, opacity 0.2s; }
